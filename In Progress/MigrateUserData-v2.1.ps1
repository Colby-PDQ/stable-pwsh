...
while (!$validCreds) {
    $cred = Get-Credential -Message "Please input administrative credentials"
    try {
...
while ($Loop) {
    ...
    while ( $ConfirmEntry -ne 'y' ) {
        ...
        $HostComputer = Read-Host -Prompt 'Enter the HOST (old computer) name'
        ...
        $DestComputer = Read-Host -Prompt 'Enter the DESTINATION (new computer) name'
